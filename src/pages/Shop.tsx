...
const Shop = () => {
  ...
  const { filters, setCategory, setSort } = useFilter(); // Remove setSearchQuery
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      ...
      try {
        ...
        console.log("[Shop] Is searching:", isSearching, "Search term:", localSearch);
        
        // If we're in search mode, use semantic search
        if (isSearching && localSearch.trim()) {
          console.log("[Shop] Searching for:", localSearch);
          const searchResults = await searchProductsSemantic(localSearch, 20);
        ...
        // Otherwise use regular filtering (show all products)
    ...
    fetchProducts();
  }, [filters, isSearching, localSearch]);

  useEffect(() => {
    const handler = () => {
      searchInputRef.current?.focus();
    };
    window.addEventListener('trendzone:focus-shop-search', handler as EventListener);
    return () => window.removeEventListener('trendzone:focus-shop-search', handler as EventListener);
  }, []);
  ...
  const getSortValue = () => {
    if (filters.sortBy === "default") return "default";
    return `${filters.sortBy}-${filters.sortOrder}`;
  };

  const handleSortChange = (value: string) => {
    if (value === "default") {
      setSort("default", "desc");
    } else {
  ...
  return (
    <div className="min-h-screen bg-background">
      <Navbar />
      <main className="container mx-auto px-4 sm:px-6 lg:px-12 max-w-[1440px] py-8">
        {/* Header */}
        <div className="mb-8">
          <h1 className="font-display font-bold text-foreground text-center mb-4" style={{ fontSize: "clamp(2rem, 4vw, 3rem)" }}>
            Shop
          </h1>
        ...
        {/* Search Bar */}
        <div className="mt-4 flex gap-2 items-center max-w-md mx-auto">
